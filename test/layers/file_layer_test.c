...
void mk_multiple_files_test()
{
    ...
    {
        // Generate file path
        snprintf(path, 15, "/test/%ld.txt", i);
        // Create file
...
void unlink_multiple_files_test()
{
    ...
    for (ssize_t i = 0; i < 2000; i++)
    {
        ...
        snprintf(filePath, 15, "/test/%ld.txt", i);
    ...
    ssize_t dirInodeNum = get_inode_num_from_path("/test");
    ...
    if (dirInode->num_blocks != 1)
    {
        ...
        printf("FAILED: EXPECTED 1 BLOCK, ACTUAL: %ld\n", dirInode->num_blocks);
        exit(-1);
    }
...
void mk_file_test()
{
    ...
    for (ssize_t i = 0; i < DIRECT_B_COUNT; i++)
    {
        ...
        ssize_t inode_num = get_inode_num_from_path(file_path);
        if (inode_num == -1)
        {
            printf("Failed to find file: %s\n", file_path);
            exit(-1);
        }
        printf("Found file: %s, inode number: %ld\n", file_path, inode_num);
...
void two_block_write_test()
{
    ...
    { // iterating through blocks a, b, c, ...
        ...
        char *buffer = "This is a test buffer to test the writing functionality of custom_write";
        ssize_t nbytes = strlen(buffer);     // get buffer size
        ssize_t num_writes = BLOCK_SIZE / nbytes + 1; // calculate number of writes to write two blocks
        for (ssize_t j = 0; j < num_writes; j++)
        { // iterate for each write
            if (custom_write(path, buffer, nbytes, j * nbytes) == -1)
            { // custom_write returns -1 if write fails
                printf("Failed to write to file %s at offset %ld\n", path, j * nbytes);
            ...
            { // read back from file
                printf("Failed to read file %s at offset %ld\n", path, j * nbytes);
...
void single_indirect_block_write_test()
{
    ...
    for (ssize_t i = 0; i < DIRECT_B_COUNT; i++)
    {
        ...
        ssize_t  nbytes = strlen(buf);
        ssize_t num_writes = BLOCK_SIZE * DIRECT_B_COUNT / nbytes + 1;
        for (ssize_t j = 0; j < num_writes; j++)
        {
...
void double_indirect_block_write_test()
{
    ...
    {                                          // iterate over direct blocks (a, b, c, ...)
        ...
        ssize_t  data_size = strlen(data);                                                        // get size of data
        ssize_t num_writes = BLOCK_SIZE * (DIRECT_B_COUNT + SINGLE_INDIRECT_BLOCK_COUNT) / data_size + 1; // calculate number of writes
        for (ssize_t j = 0; j < num_writes; j++)                                                          // write to file multiple times
        {
        ...
        printf("Checking if file size and number of blocks were updated...");
        ssize_t inode_num = get_inode_num_from_path(path); // get inode number of file
        assert(inode_num != -1);
...
void truncate_test()
{
    ...
    for (ssize_t i = 0; i < DIRECT_B_COUNT; i++)
    {
        ...
        for (ssize_t j = 1; j < DIRECT_B_COUNT; j++)
        {
            if (file_inode->direct_blocks[j] != 0)
            {
                printf("FAILED\nExpected direct_block[%ld] to be 0, Actual: %ld\n", j, file_inode->direct_blocks[j]);
                exit(-1);
            }
        }
        char *dblock = read_dblock(file_inode->direct_blocks[0]);
        for (ssize_t i = 500; i < BLOCK_SIZE; i++)
        {
            if (dblock[i] != 0)
            {
                printf("FAILED\nExpected first dblocl[%ld] to be 0, Actual: %ld\n", i, dblock[i]);
                exit(-1);
            }
        }
        printf("Success\n");
...
void unlink_files_test()
{
    ...
    for (ssize_t i = 0; i < DIRECT_B_COUNT; i++)
    {
        ...
        ssize_t inum = get_inode_num_from_path(file_path);
        if (inum != -1)
        {
...
void unlink_full_dir_test()
{
    ...
    for (ssize_t dir_num = 0; dir_num < DIRECT_B_COUNT; dir_num++)
    {
...
void unlink_empty_dir_test()
{
    ...
    for (ssize_t i = 0; i < DIRECT_B_COUNT; i++)
    {
        ...
        ssize_t unlink_result = custom_unlink(path);
        if (unlink_result < 0)
        {
...
void unlink_root_test()
{
    ...
    ssize_t result = custom_unlink("/");
    printf("result:%ld\n",result);
    // Check the return value of custom_unlink() and print appropriate message.
    if (result != -EINVAL)
    {
        printf("UNLINK ROOT test: FAILED with error code %ld\n", result);
        exit(-1);
    }
    else
    {
